									...
									<span class="thumb-info thumb-info-hide-wrapper-bg mb-xlg">
										<span class="thumb-info-wrapper">
											<a href="{{ route('logotipu-kurimas.disainer.view', $disainer->id) }}">
												...
												@if( ($disainer->disainer_avatar) == '')
													<img src="{{ url('assets/img/misc/team-1.jpg') }}" class="img-responsive" alt="{{$disainer->disainer_name}}">
												@else
													<img src="/storage/disainer/{{$disainer->disainer_avatar}}" class="img-responsive" alt="{{$disainer->disainer_name}}">
												@endif
										...
										<div class="thumb-info-caption">
											...
											<ul class="list list-icons list-icons-style-3 mt-xs pl-md pr-md">
												<li><i class="fa fa-phone"></i>  {{$disainer->disainer_phone}}</li>
												<li><i class="fa fa-envelope"></i> <a href="mailto:{{$disainer->disainer_email}}"> {{$disainer->disainer_email}}</a></li>
                    						</ul>
											<hr>
											<p class="center"><a href="{{ route('logotipu-kurimas.disainer.view', $disainer->id) }}"><button type="button" class="btn btn-3d btn-primary">Detaliau</button></a></p>
										</div>
									</span>
                        </li>
                            @endif
